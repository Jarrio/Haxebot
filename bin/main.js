...
Main.start = function() {
	...
	var vec1 = this1;
	vec1[2] = new ecs_Components(6);
	vec1[1] = new ecs_Components(6);
	vec1[5] = new ecs_Components(6);
	vec1[3] = new ecs_Components(6);
	vec1[4] = new ecs_Components(6);
	vec1[0] = new ecs_Components(6);
	...
	Main.client.once("ready",function() {
		...
		res.then(function(foo) {
			...
			while(_g < foo.length) {
				var item = foo[_g];
				++_g;
				haxe_Log.trace("" + Std.string(item.name) + " registered",{ fileName : "src/Main.hx", lineNumber : 117, className : "Main", methodName : "start"});
			}
		},Main.err);
	});
	Main.client.on("messageCreate",function(message) {
		...
		if(channel.type == "DM") {
			if(Object.prototype.hasOwnProperty.call(Main.dm_help_tracking.h,message.author.id)) {
				...
				Main.universe.components.set(_ecsTmpEntity,5,"helppls");
		...
		if(channel.type == "GUILD_TEXT") {
			...
			if(StringTools.startsWith(message.content,"!react")) {
				var _ecsTmpEntity = Main.universe.createEntity();
				Main.universe.components.set(_ecsTmpEntity,5,"react");
				Main.universe.components.set(_ecsTmpEntity,1,message);
				var ecsEntCompFlags = Main.universe.components.flags[ecs_Entity.id(_ecsTmpEntity)];
				var ecsTmpFamily = Main.universe.families.get(3);
				if(bits_Bits.areSet(ecsEntCompFlags,ecsTmpFamily.componentsMask)) {
		...
		var _ecsTmpEntity = Main.universe.createEntity();
		Main.universe.components.set(_ecsTmpEntity,5,"scam_prevention");
		Main.universe.components.set(_ecsTmpEntity,1,message);
		var ecsEntCompFlags = Main.universe.components.flags[ecs_Entity.id(_ecsTmpEntity)];
		var ecsTmpFamily = Main.universe.families.get(3);
		if(bits_Bits.areSet(ecsEntCompFlags,ecsTmpFamily.componentsMask)) {
	...
	Main.client.on("ChatInputAutoCompleteEvent",function(incoming) {
		haxe_Log.trace("disconnected",{ fileName : "src/Main.hx", lineNumber : 174, className : "Main", methodName : "start"});
		haxe_Log.trace(incoming,{ fileName : "src/Main.hx", lineNumber : 175, className : "Main", methodName : "start"});
	});
	Main.client.on("interactionCreate",function(interaction) {
		if(interaction.isModalSubmit()) {
			if(interaction.customId == "showcase") {
				...
				Main.universe.components.set(_ecsTmpEntity,4,new components_ShowcaseModalSubmit(title,description));
				var ecsEntCompFlags = Main.universe.components.flags[ecs_Entity.id(_ecsTmpEntity)];
...
Main.err = function(err) {
	haxe_Log.trace(err,{ fileName : "src/Main.hx", lineNumber : 279, className : "Main", methodName : "err"});
};
Main.saveCommand = function(command) {
	Main.commands.h[command.name] = command;
	haxe_Log.trace("registered " + command.name,{ fileName : "src/Main.hx", lineNumber : 284, className : "Main", methodName : "saveCommand"});
};
Main.main = function() {
	try {
		...
		haxe_Log.trace(_g1.get_message(),{ fileName : "src/Main.hx", lineNumber : 291, className : "Main", methodName : "main"});
	}
		...
		if(command.params != null) {
			...
			while(_g1 < _g2.length) {
				...
				case "string":
					...
					if(param.name == "api") {
						haxe_Log.trace("here",{ fileName : "src/Main.hx", lineNumber : 343, className : "Main", methodName : "parseCommands"});
						haxe_Log.trace(autocomplete,{ fileName : "src/Main.hx", lineNumber : 344, className : "Main", methodName : "parseCommands"});
					}
...
var components_CommandOptions = $hxEnums["components.CommandOptions"] = { __ename__:"components.CommandOptions",__constructs__:null
	...
	,Poll: ($_=function(question,time) { return {_hx_index:7,question:question,time:time,__enum__:"components.CommandOptions",toString:$estr}; },$_._hx_name="Poll",$_.__params__ = ["question","time"],$_)
	...
	,Api: ($_=function(channel) { return {_hx_index:12,channel:channel,__enum__:"components.CommandOptions",toString:$estr}; },$_._hx_name="Api",$_.__params__ = ["channel"],$_)
	...
	,Showcase: {_hx_name:"Showcase",_hx_index:17,__enum__:"components.CommandOptions",toString:$estr}
};
...
systems_commands_Api.prototype = $extend(systems_CommandBase.prototype,{
	...
	,search: function(string,interaction) {
		...
		interaction.respond(results).then(null,function(err) {
			haxe_Log.trace(err,{ fileName : "src/systems/commands/Api.hx", lineNumber : 112, className : "systems.commands.Api", methodName : "search"});
		});
	}
	,get_name: function() {
		return "api";
	...
	,runCodeOnThread: function(code,message) {
		...
		try {
			...
			js_node_Fs.appendFile("" + this.get_base_path() + "/hx/" + filename + ".hx",code_content + ("//User:" + message.author.tag + " | time: " + Std.string(new Date())),function(error) {
				if(error != null) {
					haxe_Log.trace(error,{ fileName : "src/systems/commands/Run.hx", lineNumber : 323, className : "systems.commands.Run", methodName : "runCodeOnThread"});
				}
				...
				ls.stderr.once("data",function(data) {
					haxe_Log.trace("error: " + data,{ fileName : "src/systems/commands/Run.hx", lineNumber : 348, className : "systems.commands.Run", methodName : "runCodeOnThread"});
					var compile_output = _gthis.cleanOutput(data,filename,class_entry);
					var embed = _gthis.parseError(compile_output,code_content);
					if(embed == null) {
				...
				ls.once("close",function(data) {
					...
					if(!sys_FileSystem.exists(js_file)) {
						haxe_Log.trace("Code likely errored and didnt compile (" + filename + ".js)",{ fileName : "src/systems/commands/Run.hx", lineNumber : 367, className : "systems.commands.Run", methodName : "runCodeOnThread"});
						ls.kill("SIGTERM");
						return;
					}
					var obj = null;
					...
					try {
						...
						if(response.length > 0 && data == 0) {
							message.reply({ embeds : [embed]}).then(function(succ) {
								haxe_Log.trace("" + message.author.tag + " at " + format_date + " with file id: " + filename,{ fileName : "src/systems/commands/Run.hx", lineNumber : 444, className : "systems.commands.Run", methodName : "runCodeOnThread"});
								return message.delete().then(null,function(err) {
									haxe_Log.trace(err,{ fileName : "src/systems/commands/Run.hx", lineNumber : 446, className : "systems.commands.Run", methodName : "runCodeOnThread"});
								});
							},function(err) {
								haxe_Log.trace(err,{ fileName : "src/systems/commands/Run.hx", lineNumber : 449, className : "systems.commands.Run", methodName : "runCodeOnThread"});
							});
							ls.kill();
							return;
						}
						...
						message.reply({ content : mention + ("```\n" + compile_output + "```")});
						haxe_Log.trace(_g1,{ fileName : "src/systems/commands/Run.hx", lineNumber : 457, className : "systems.commands.Run", methodName : "runCodeOnThread"});
					}
				});
			});
			return;
			...
			haxe_NativeStackTrace.lastError = _g;
			var _g1 = haxe_Exception.caught(_g).unwrap();
			haxe_Log.trace(_g1,{ fileName : "src/systems/commands/Run.hx", lineNumber : 464, className : "systems.commands.Run", methodName : "runCodeOnThread"});
...
var systems_commands_Showcase = function(_universe) {
	...
	this.xpost = this.universe.families.get(3);
...
systems_commands_Showcase.prototype = $extend(systems_CommandBase.prototype,{
	...
	,update: function(_) {
		...
		if(this.channel == null && !this.checking) {
			this.checking = true;
			Main.client.channels.fetch("898957515654574121").then(function(channel) {
				_gthis.channel = channel;
				_gthis.checking = false;
				haxe_Log.trace("loaded showcase channel",{ fileName : "src/systems/commands/Showcase.hx", lineNumber : 27, className : "systems.commands.Showcase", methodName : "update"});
			},function(err) {
				haxe_Log.trace(err,{ fileName : "src/systems/commands/Showcase.hx", lineNumber : 28, className : "systems.commands.Showcase", methodName : "update"});
			});
		}
		...
		while(_active && _g1_idx >= 0) {
			...
			var thread = js_Boot.__cast(message.channel , discord_$js_ThreadChannel);
			if(thread.ownerId != message.author.id) {
				return;
			}
			var content = StringTools.trim(message.content.substring(10));
			var forum = thread.id;
			this.channel.send("Showcase: <#" + forum + "> \nBy: <@" + message.author.id + ">\n\n" + content).then(null,function(err) {
	...
	,run: function(command,interaction) {
		var modal = new discord_$builder_ModalBuilder().setCustomId("showcase").setTitle("Showcase");
		var title_input = new discord_$builder_APITextInputComponent().setCustomId("titlelink").setLabel("Title or Link").setStyle(1).setRequired(true);
		var description_input = new discord_$builder_APITextInputComponent().setCustomId("description").setLabel("Description").setRequired(false).setStyle(2);
		var row_1 = new discord_$builder_APIActionRowComponent().addComponents(title_input);
		var row_2 = new discord_$builder_APIActionRowComponent().addComponents(description_input);
		modal.addComponents(row_1,row_2);
		interaction.showModal(modal).then(function(succ) {
			haxe_Log.trace("win",{ fileName : "src/systems/commands/Showcase.hx", lineNumber : 67, className : "systems.commands.Showcase", methodName : "run"});
		},function(err) {
			haxe_Log.trace(err,{ fileName : "src/systems/commands/Showcase.hx", lineNumber : 67, className : "systems.commands.Showcase", methodName : "run"});
		});
	}
	,reply: function(command,interaction) {
	}
	,get_name: function() {
		return "showcase";
	}
...
systems_commands_Trace.prototype = $extend(systems_CommandBase.prototype,{
	...
	,runCode: function(code,interaction) {
		...
		js_node_Fs.appendFile("" + this.get_base_path() + "/hx/" + filename + ".hx",final_code + ("\n//User:" + interaction.user.tag + " id: " + interaction.user.id + "| time: " + Std.string(new Date())),function(error) {
			if(error != null) {
				haxe_Log.trace(error,{ fileName : "src/systems/commands/Trace.hx", lineNumber : 118, className : "systems.commands.Trace", methodName : "runCode"});
			}
			var commands = ["-cp","" + _gthis.get_base_path() + "/hx","-main",filename,"-js","" + _gthis.get_base_path() + "/bin/" + filename + ".js"];
			var $process = "./haxe/haxe";
			if(!sys_FileSystem.exists("./haxe/haxe")) {
			...
			ls.stderr.once("data",function(data) {
				haxe_Log.trace("error: " + data,{ fileName : "src/systems/commands/Trace.hx", lineNumber : 138, className : "systems.commands.Trace", methodName : "runCode"});
			...
			ls.once("close",function(data) {
				...
				var js_file = "" + _gthis.get_base_path() + "/bin/" + filename + ".js";
				if(!sys_FileSystem.exists(js_file)) {
					haxe_Log.trace("Code likely errored and didnt compile (" + filename + ".js)",{ fileName : "src/systems/commands/Trace.hx", lineNumber : 157, className : "systems.commands.Trace", methodName : "runCode"});
					return;
				}
				var obj = null;
				var vm = new vm2_NodeVM({ sandbox : obj, console : "redirect", timeout : _gthis.timeout});
				...
				try {
					...
					if(response.length > 0 && data == 0) {
						interaction.reply({ embeds : [embed]}).then(function(succ) {
							haxe_Log.trace("" + interaction.user.tag + "(" + interaction.user.id + ") at " + format_date + " with file id: " + filename,{ fileName : "src/systems/commands/Trace.hx", lineNumber : 225, className : "systems.commands.Trace", methodName : "runCode"});
						},function(err) {
							haxe_Log.trace(err,{ fileName : "src/systems/commands/Trace.hx", lineNumber : 227, className : "systems.commands.Trace", methodName : "runCode"});
						});
						ls.kill();
						return;
					}
					...
					interaction.reply({ content : mention + ("```\n" + compile_output + "```")});
					haxe_Log.trace(_g1,{ fileName : "src/systems/commands/Trace.hx", lineNumber : 235, className : "systems.commands.Trace", methodName : "runCode"});
				}
			});
		});
	}
