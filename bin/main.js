...
Main.start = function() {
	...
	Main.client.once("ready",function() {
		...
		haxe_Log.trace("Ready!",{ fileName : "src/Main.hx", lineNumber : 108, className : "Main", methodName : "start"});
		...
		res.then(function(foo) {
			...
			while(_g < foo.length) {
				var item = foo[_g];
				++_g;
				haxe_Log.trace("" + Std.string(item.name) + " registered",{ fileName : "src/Main.hx", lineNumber : 118, className : "Main", methodName : "start"});
			}
		},Main.err);
	});
...
var haxe_macro_CompilationServer = function() { };
$hxClasses["haxe.macro.CompilationServer"] = haxe_macro_CompilationServer;
haxe_macro_CompilationServer.__name__ = "haxe.macro.CompilationServer";
	...
	run: function(command,interaction) {
		...
		if(_g._hx_index == 2) {
			...
			member.guild.channels.fetch(null,{ force : true}).then(function(response) {
				...
				var _g_current = 0;
				var _g_array = channels;
				while(_g_current < _g_array.length) {
					var _g_value = _g_array[_g_current];
					var _g_key = _g_current++;
					var key = [_g_key];
					if(_g_value.type != "GUILD_TEXT") {
						break;
					}
					Main.client.channels.fetch(_g_value.id,{ force : true}).then((function(key) {
						return function(channel) {
