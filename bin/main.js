...
Main.start = function() {
	...
	Main.universe.systems.add(new systems_commands_Helppls(Main.universe));
	...
	Main.client = new discord_$js_Client({ intents : ["GUILDS","GUILD_MESSAGES","DIRECT_MESSAGES"]});
	...
	Main.client.on("interactionCreate",function(interaction) {
		...
		if(command.content == null) {
			haxe_Log.trace(interaction,{ fileName : "src/Main.hx", lineNumber : 116, className : "Main", methodName : "start"});
			haxe_Log.trace(enum_id,{ fileName : "src/Main.hx", lineNumber : 117, className : "Main", methodName : "start"});
			haxe_Log.trace("Unmatched command. (" + command.name + ")",{ fileName : "src/Main.hx", lineNumber : 118, className : "Main", methodName : "start"});
		}
		var _ecsTmpEntity = Main.universe.entities.create();
...
Main.main = function() {
	try {
		...
		haxe_Log.trace(_g1.get_message(),{ fileName : "src/Main.hx", lineNumber : 134, className : "Main", methodName : "main"});
	}
	...
	rest.put(Routes.applicationGuildCommands(Main.config.client_id,Main.config.server_id),{ body : commands}).then(function(_) {
		haxe_Log.trace("Successfully registered application commands.",{ fileName : "src/Main.hx", lineNumber : 146, className : "Main", methodName : "main"});
	},function(err) {
		haxe_Log.trace(err,{ fileName : "src/Main.hx", lineNumber : 146, className : "Main", methodName : "main"});
	});
	Main.start();
};
...
systems_commands_Helppls.__super__ = systems_CommandBase;
systems_commands_Helppls.prototype = $extend(systems_CommandBase.prototype,{
	onAdded: function() {
		systems_CommandBase.prototype.onAdded.call(this);
					...
					try {
						...
						if(response.length > 0 && data == 0) {
							message.reply({ embeds : [embed]}).then(function(succ) {
								haxe_Log.trace("" + message.author.tag + " at " + format_date + " with file id: " + filename,{ fileName : "src/systems/commands/Run.hx", lineNumber : 361, className : "systems.commands.Run", methodName : "runCodeOnThread"});
						...
						var _g1 = haxe_Exception.caught(_g);
						haxe_Log.trace(_g1,{ fileName : "src/systems/commands/Run.hx", lineNumber : 368, className : "systems.commands.Run", methodName : "runCodeOnThread"});
					}
				});
			});
			return;
