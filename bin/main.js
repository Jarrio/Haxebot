// Generated by Haxe 5.0.0-alpha.1+0adc110
...
Main.start = function() {
	...
	vec[1] = new ecs_Phase(false,"testing",new Array(14),new Array(14));
	vec[2] = new ecs_Phase(true,"main",new Array(31),new Array(31));
	var phases = vec;
	...
	s.onEnabled();
	var phase = phases[1];
	var s = new commands_ScheduleEventPoster(u);
	phase.systems[0] = s;
	phase.enabledSystems[0] = true;
	...
	Main.universe = u;
	Main.client = new discord_$js_Client({ intents : [1,32768,65536,512,4096,2,1024,128,8192]});
	Main.client.once("ready",function() {
		var $l=arguments.length;
		var clients = new Array($l>0?$l-0:0);
		for(var $i=0;$i<$l;++$i){clients[$i-0]=arguments[$i];}
		haxe_Log.trace("Ready!",{ fileName : "src/Main.hx", lineNumber : 171, className : "Main", methodName : "start"});
		...
		res.then(function(foo) {
			...
			while(_g < foo.length) {
				var item = foo[_g];
				++_g;
				haxe_Log.trace("DEBUG - " + item.name + " is REGISTERED",{ fileName : "src/Main.hx", lineNumber : 180, className : "Main", methodName : "start"});
			}
		},function(err) {
			haxe_Log.trace(err,{ fileName : "src/Main.hx", lineNumber : 186, className : "Main", methodName : "start"});
			$global.console.dir(err);
		});
	});
	Main.client.on("guildMemberAdd",function(member) {
		haxe_Log.trace("member " + member.user.tag,{ fileName : "src/Main.hx", lineNumber : 192, className : "Main", methodName : "start"});
		var _ecsTmpEntity = Main.universe.createEntity();
		Main.universe.components.set(_ecsTmpEntity,2,"add_event_role");
		Main.universe.components.set(_ecsTmpEntity,6,member);
	...
	Main.client.on("guildScheduledEventCreate",function(event) {
		...
		Main.universe.components.set(_ecsTmpEntity,3,event);
		var ecsEntCompFlags = Main.universe.components.flags[ecs_Entity.id(_ecsTmpEntity)];
		var ecsTmpFamily = Main.universe.families.get(1);
		if(bits_Bits.areSet(ecsEntCompFlags,ecsTmpFamily.componentsMask)) {
			ecsTmpFamily.add(_ecsTmpEntity);
	...
	Main.client.on("threadCreate",function(thread) {
		...
		Main.universe.components.set(_ecsTmpEntity,5,thread);
		var ecsEntCompFlags = Main.universe.components.flags[ecs_Entity.id(_ecsTmpEntity)];
		var ecsTmpFamily = Main.universe.families.get(1);
		if(bits_Bits.areSet(ecsEntCompFlags,ecsTmpFamily.componentsMask)) {
			ecsTmpFamily.add(_ecsTmpEntity);
	...
	Main.client.on("interactionCreate",function(interaction) {
		...
		if(interaction.isModalSubmit()) {
			haxe_Log.trace("here",{ fileName : "src/Main.hx", lineNumber : 253, className : "Main", methodName : "start"});
			haxe_Log.trace(interaction.customId,{ fileName : "src/Main.hx", lineNumber : 254, className : "Main", methodName : "start"});
...
var commands_AutoRole = function(_universe) {
	...
	this.tablec254489e95ef23a8f91062a1947780b9 = this.universe.components.getTable(6);
};
		...
		if(_g._hx_index == 29) {
			...
			ls.stdout.once("close",function(data) {
				...
				return interaction.reply({ embeds : [embed]}).then(null,function(err) {
					haxe_Log.trace(err,{ fileName : "src/commands/Haxelib.hx", lineNumber : 80, className : "commands.Haxelib", methodName : "run"});
					$global.console.dir(err);
				});
			});
				...
				if(found) {
					...
					interaction.member.roles.add(role).then(function(success) {
						if(channel == "announcements") {
							...
							_gthis.universe.components.set(_ecsTmpEntity,6,member);
...
commands_Roundup.prototype = $extend(systems_CommandBase.prototype,{
	...
	,dmUser: function(title,content) {
		...
		while(_g_current < _g_length) {
			...
			var user = [_g_value];
			if(user[0] == null) {
				haxe_Log.trace("skipping " + key,{ fileName : "src/commands/Roundup.hx", lineNumber : 33, className : "commands.Roundup", methodName : "dmUser"});
				continue;
			}
			user[0].send("## " + title);
			...
			user[0].send(a).then((function(b,user) {
				return function(_) {
					return user[0].send(b[0]).then(null,(function() {
						return function(err) {
							haxe_Log.trace(err,{ fileName : "src/commands/Roundup.hx", lineNumber : 49, className : "commands.Roundup", methodName : "dmUser"});
						};
					})());
				};
			})(b,user),(function() {
				return function(err) {
					haxe_Log.trace(err,{ fileName : "src/commands/Roundup.hx", lineNumber : 50, className : "commands.Roundup", methodName : "dmUser"});
				};
			})());
		}
	}
	,getHaxeIoPage: function() {
		...
		data.onError = function(error) {
			haxe_Log.trace(error,{ fileName : "src/commands/Roundup.hx", lineNumber : 60, className : "commands.Roundup", methodName : "getHaxeIoPage"});
		};
		data.onData = function(body) {
			...
			if(regex.match(body)) {
				...
				desc += "\n...";
				embed.setDescription(desc);
				_gthis.dmUser("Haxe Roundup #" + Main.get_state().next_roundup,desc);
	...
	,update: function(_) {
		...
		if(this.channel == null && this.checking == false) {
			...
			Main.client.channels.fetch(this.announcement_channel).then(function(channel) {
				...
				haxe_Log.trace(err,{ fileName : "src/commands/Roundup.hx", lineNumber : 105, className : "commands.Roundup", methodName : "update"});
				$global.console.dir(err);
			});
			var h = this.dmlist.h;
			var _g_h = h;
				...
				if(user == null) {
					Main.client.users.fetch(key1[0]).then((function(key) {
						return function(user) {
							...
							haxe_Log.trace("Got " + user.tag,{ fileName : "src/commands/Roundup.hx", lineNumber : 113, className : "commands.Roundup", methodName : "update"});
						};
					})(key1),(function() {
						return function(err) {
							haxe_Log.trace(err,{ fileName : "src/commands/Roundup.hx", lineNumber : 114, className : "commands.Roundup", methodName : "update"});
						};
					})());
				}
			}
	...
	,run: function(command,interaction) {
		...
		if(_g._hx_index == 22) {
			...
			interaction.client.channels.fetch(this.announcement_channel).then(function(channel) {
				...
				haxe_Log.trace(err,{ fileName : "src/commands/Roundup.hx", lineNumber : 190, className : "commands.Roundup", methodName : "run"});
				$global.console.dir(err);
			});
		}
	}
...
var commands_RoundupRoundup = function(_universe) {
	...
	this.scheduled_event_updates = this.universe.families.get(1);
	...
	this.table87a8f92f715c03d0822a55d9b93a210d = this.universe.components.getTable(2);
	this.tablec9adfcb69cacf935dab274fb9ef32870 = this.universe.components.getTable(3);
};
	...
	,update: function(_) {
		...
		while(_active && _g_idx >= 0) {
			...
			if(forward == "roundup_member_update") {
				if(this.event != null && this.event.status == 2) {
					...
					if(member.id == this.get_host() && updated.channel == null) {
		...
		if(this.voice_channel == null && !this.waiting) {
			...
			Main.client.channels.fetch(this.voice_channel_id).then(function(channel) {
				...
				haxe_Log.trace(err,{ fileName : "src/commands/RoundupRoundup.hx", lineNumber : 69, className : "commands.RoundupRoundup", methodName : "update"});
			});
		}
		...
		if(this.host_m == null && !this.waiting) {
			...
			this.get_guild().members.fetch(this.get_state().host).then(function(member) {
				...
				_gthis.waiting = false;
				haxe_Log.trace("Roundup host obtained",{ fileName : "src/commands/RoundupRoundup.hx", lineNumber : 105, className : "commands.RoundupRoundup", methodName : "update"});
			},function(err) {
				haxe_Log.trace(err,{ fileName : "src/commands/RoundupRoundup.hx", lineNumber : 106, className : "commands.RoundupRoundup", methodName : "update"});
			});
		}
		if(this.host_m == null) {
			return;
...
var systems_TextCommandBase = function(_universe) {
	ecs_System.call(this,_universe);
	this.commands = this.universe.families.get(6);
	this.tabled1cd3067ebd0108e92f1425a40ea7b45 = this.universe.components.getTable(4);
...
var commands_Run = function(_universe) {
	...
	this.code_messages = this.universe.families.get(6);
};
...
commands_ScamPrevention.prototype = $extend(systems_CommandBase.prototype,{
		...
		if(!(new Date().getTime() - this.phishing_update_time < 21600000)) {
			...
			links.onData = function(data) {
				try {
					...
					var _g1 = haxe_Exception.caught(_g);
					haxe_Log.trace(_g1,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 128, className : "commands.ScamPrevention", methodName : "getPhishingLinks"});
					haxe_Log.trace("error parsing phishing links",{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 129, className : "commands.ScamPrevention", methodName : "getPhishingLinks"});
		...
		this.timeoutUser(message,function(_) {
			message.reply({ content : "<@&198916468312637440> Please review this message by: <@" + message.author.id + ">", embeds : [embed]}).then(function(_) {
				...
				while(_g < messages.length) {
					...
					++_g;
					message.delete().then(null,function(err) {
						haxe_Log.trace(err,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 99, className : "commands.ScamPrevention", methodName : "reviewMessage"});
						$global.console.dir(err);
					});
				}
			},function(err) {
				haxe_Log.trace(err,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 104, className : "commands.ScamPrevention", methodName : "reviewMessage"});
				$global.console.dir(err);
			});
		});
	}
	...
	,timeoutUser: function(message,callback) {
		...
		message.guild.members.fetch(message.author.id).then(function(guild_member) {
			...
			guild_member.timeout(43200000,"Stop spamming, a mod will review this at their convenience.").then(callback,function(err) {
				haxe_Log.trace(err,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 145, className : "commands.ScamPrevention", methodName : "timeoutUser"});
				$global.console.dir(err);
			});
			...
			haxe_Log.trace(err,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 150, className : "commands.ScamPrevention", methodName : "timeoutUser"});
			$global.console.dir(err);
		});
	}
	...
	,banUser: function(messages,callback) {
		...
		message.guild.members.fetch(message.author.id).then(function(guild_member) {
			...
			guild_member.ban({ days : 1, reason : "found phishing links, auto banned."}).then(null,function(err) {
				haxe_Log.trace(err,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 180, className : "commands.ScamPrevention", methodName : "banUser"});
				$global.console.dir(err);
			});
			var id = message.author.id;
			var _this = _gthis.time_since;
			...
			message.channel.send("User <@" + message.author.id + "> has been auto banned for sending scam links.").then(callback,function(err) {
				haxe_Log.trace(err,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 188, className : "commands.ScamPrevention", methodName : "banUser"});
				$global.console.dir(err);
			});
		},function(err) {
			haxe_Log.trace(err,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 192, className : "commands.ScamPrevention", methodName : "banUser"});
			$global.console.dir(err);
		});
	}
	,logMessage: function(id,embed,action) {
		...
		Main.client.channels.fetch("952952631079362650").then(function(channel) {
			...
			haxe_Log.trace(err,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 203, className : "commands.ScamPrevention", methodName : "logMessage"});
			$global.console.dir(err);
		});
	}
				...
				if(message.content.indexOf(link) != -1) {
					...
					if(regex.match(message.content)) {
						...
						if(url.hostname.length == 0 || url.hostname == null) {
							haxe_Log.trace(regex.matched(1),{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 240, className : "commands.ScamPrevention", methodName : "checkPhishingLinks"});
							return false;
						}
	...
	,checkEquality: function(messages) {
		...
		while(_g < messages.length) {
			...
			try {
				...
				haxe_Log.trace(_g2,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 290, className : "commands.ScamPrevention", methodName : "checkEquality"});
				haxe_Log.trace(JSON.stringify(messages),{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 291, className : "commands.ScamPrevention", methodName : "checkEquality"});
			}
		}
...
var commands_ScheduleEventPoster = function(_universe) {
	this.stream_channel_id = "1017520941661687878";
	this.connected = false;
	this.client = new http_HttpClient();
	this.once = false;
	systems_CommandBase.call(this,_universe);
	this.events = this.universe.families.get(1);
	this.table87a8f92f715c03d0822a55d9b93a210d = this.universe.components.getTable(2);
	this.tablec9adfcb69cacf935dab274fb9ef32870 = this.universe.components.getTable(3);
};
$hxClasses["commands.ScheduleEventPoster"] = commands_ScheduleEventPoster;
commands_ScheduleEventPoster.__name__ = "commands.ScheduleEventPoster";
commands_ScheduleEventPoster.__super__ = systems_CommandBase;
commands_ScheduleEventPoster.prototype = $extend(systems_CommandBase.prototype,{
	channel: null
	,once: null
	,client: null
	,connected: null
	...
	,getToken: function() {
		var _gthis = this;
		var url = new http_HttpRequestObject(http_Url.fromString("https://id.twitch.tv/oauth2/token"));
		url.get_queryParams().h["client_id"] = Main.keys.twitch_client_id;
		url.get_queryParams().h["client_secret"] = Main.keys.twitch_secret;
		url.get_queryParams().h["grant_type"] = "client_credentials";
		url.method = "POST";
		thenshim_Promise.then(this.client.makeRequest(url),function(resp) {
			_gthis.bearer = resp.response.get_bodyAsJson();
			haxe_Log.trace("got token",{ fileName : "src/commands/ScheduleEventPoster.hx", lineNumber : 30, className : "commands.ScheduleEventPoster", methodName : "getToken"});
			_gthis.subscribeChannel();
		},function(err) {
			haxe_Log.trace(err,{ fileName : "src/commands/ScheduleEventPoster.hx", lineNumber : 32, className : "commands.ScheduleEventPoster", methodName : "getToken"});
		});
	}
	,onEnabled: function() {
		this.getToken();
	...
	,subscribeChannel: function() {
		var request = new http_HttpRequestObject(http_Url.fromString("https://api.twitch.tv/helix/eventsub/subscriptions"));
		request.body = { type : "channel.update", version : 2, condition : { broadcaster_user_id : "55270689"}, transport : { method : "webhook", callback : "https://discord.com/api/webhooks/1235572910291419137/FcVYJauOCwQsiLRnc6PT3ePU-WPXV9QeGoGItYwzkTLR5Br4mXcmWijCCktKDv972O4I", secret : Main.keys.twitch_secret}};
		request.method = "POST";
		request.headers = new haxe_ds_StringMap();
		request.headers.h["Authorization"] = "Bearer " + this.bearer.access_token;
		request.headers.h["Client-Id"] = Main.keys.twitch_client_id;
		request.headers.h["Content-Type"] = "application/json";
		thenshim_Promise.then(this.client.makeRequest(request),function(inc) {
			haxe_Log.trace(inc.response.headers == null ? "null" : haxe_ds_StringMap.stringify(inc.response.headers.h),{ fileName : "src/commands/ScheduleEventPoster.hx", lineNumber : 61, className : "commands.ScheduleEventPoster", methodName : "subscribeChannel"});
			haxe_Log.trace(inc.response.get_bodyAsString(),{ fileName : "src/commands/ScheduleEventPoster.hx", lineNumber : 62, className : "commands.ScheduleEventPoster", methodName : "subscribeChannel"});
		},function(err) {
			haxe_Log.trace(err,{ fileName : "src/commands/ScheduleEventPoster.hx", lineNumber : 63, className : "commands.ScheduleEventPoster", methodName : "subscribeChannel"});
		});
	}
	,update: function(_) {
		var _gthis = this;
		...
		if(!this.once) {
			this.once = true;
			util_DiscordUtil.getChannel(this.stream_channel_id,function(channel) {
				haxe_Log.trace("stream channel received",{ fileName : "src/commands/ScheduleEventPoster.hx", lineNumber : 71, className : "commands.ScheduleEventPoster", methodName : "update"});
				_gthis.channel = channel;
			});
		}
		...
		while(_active && _g_idx >= 0) {
			...
			var forward = this.table87a8f92f715c03d0822a55d9b93a210d.get(entity);
			var event = this.tablec9adfcb69cacf935dab274fb9ef32870.get(entity);
			if(forward == "create_event") {
				if(event.channelId != null) {
					event.createInviteURL({ maxAge : 0, channel : this.stream_channel_id}).then($bind(this,this.sendUrl),function(err) {
						haxe_Log.trace(err,{ fileName : "src/commands/ScheduleEventPoster.hx", lineNumber : 85, className : "commands.ScheduleEventPoster", methodName : "update"});
					});
				}
				var tmp = event != null ? event.entityMetadata : null;
				if(tmp != null ? tmp.location.indexOf("twitch") != -1 : null) {
					event.createInviteURL({ maxAge : 0, channel : this.stream_channel_id}).then($bind(this,this.sendUrl),function(err) {
						haxe_Log.trace(err,{ fileName : "src/commands/ScheduleEventPoster.hx", lineNumber : 90, className : "commands.ScheduleEventPoster", methodName : "update"});
					});
				}
			}
			this.universe.deleteEntity(entity);
	...
	,sendUrl: function(url) {
		this.channel.send({ content : url}).then(null,function(err) {
			haxe_Log.trace(err,{ fileName : "src/commands/ScheduleEventPoster.hx", lineNumber : 101, className : "commands.ScheduleEventPoster", methodName : "sendUrl"});
		});
	}
	,run: function(command,interaction) {
	}
	...
	,table87a8f92f715c03d0822a55d9b93a210d: null
	,tablec9adfcb69cacf935dab274fb9ef32870: null
	,__class__: commands_ScheduleEventPoster
});
var commands_Showcase = function(_) {
	this.checking = false;
	this.channel_id = "162664383082790912";
	systems_CommandBase.call(this,_);
	this.modal = this.universe.families.get(11);
	this.messages = this.universe.families.get(3);
	this.interactions = this.universe.families.get(7);
	this.table57fe33dae47d23e66b521963cf6643b9 = this.universe.components.getTable(11);
	this.table87a8f92f715c03d0822a55d9b93a210d = this.universe.components.getTable(2);
	this.tabled1cd3067ebd0108e92f1425a40ea7b45 = this.universe.components.getTable(4);
...
commands_Trace.prototype = $extend(systems_CommandBase.prototype,{
	...
	,update: function(_) {
		...
		if(!(now - this.last_cleared < 604800000)) {
			...
			try {
				...
				var _g1 = haxe_Exception.caught(_g);
				haxe_Log.trace(_g1,{ fileName : "src/commands/Trace.hx", lineNumber : 66, className : "commands.Trace", methodName : "cleanDirectory"});
			}
		}
	}
	,cleanDirectory: function() {
	...
	,run: function(command,interaction) {
		...
		if(_g._hx_index == 19) {
			var _gcode = _g.code;
			if(!this.isSafe(_gcode,interaction)) {
				interaction.reply("That code is not safe.").then(null,function(err) {
					haxe_Log.trace(err,{ fileName : "src/commands/Trace.hx", lineNumber : 114, className : "commands.Trace", methodName : "run"});
				});
				return;
			}
			this.runCode(_gcode,interaction);
		}
	}
	,runCode: function(code,interaction) {
		...
		js_node_Fs.appendFile("" + this.get_base_path() + "/hx/" + filename + ".hx",final_code + ("\n//User:" + interaction.user.tag + " id: " + interaction.user.id + "| time: " + Std.string(new Date())),function(error) {
			if(error != null) {
				haxe_Log.trace(error,{ fileName : "src/commands/Trace.hx", lineNumber : 135, className : "commands.Trace", methodName : "runCode"});
			}
			var commands = ["-cp","" + _gthis.get_base_path() + "/hx","-main",filename,"-js","" + _gthis.get_base_path() + "/bin/" + filename + ".js"];
			var $process = "./haxe/haxe";
			if(!sys_FileSystem.exists($process)) {
			...
			ls.stderr.once("data",function(data) {
				haxe_Log.trace("error: " + (data == null ? "null" : "" + data),{ fileName : "src/commands/Trace.hx", lineNumber : 155, className : "commands.Trace", methodName : "runCode"});
				...
				if(embed == null) {
					interaction.reply({ allowedMentions : { parse : []}, content : mention + ("```\n" + compile_output + "```")}).then(null,function(err) {
						haxe_Log.trace(err,{ fileName : "src/commands/Trace.hx", lineNumber : 164, className : "commands.Trace", methodName : "runCode"});
					});
				} else {
					...
					interaction.reply({ allowedMentions : { parse : []}, embeds : [embed]}).then(null,function(err) {
						haxe_Log.trace(err,{ fileName : "src/commands/Trace.hx", lineNumber : 169, className : "commands.Trace", methodName : "runCode"});
					});
				}
				ls.kill("SIGTERM");
			});
			ls.once("close",function(data) {
				...
				var js_file = "" + _gthis.get_base_path() + "/bin/" + filename + ".js";
				if(!sys_FileSystem.exists(js_file)) {
					haxe_Log.trace("Code likely errored and didnt compile (" + filename + ".js)",{ fileName : "src/commands/Trace.hx", lineNumber : 180, className : "commands.Trace", methodName : "runCode"});
					return;
				}
				var obj = null;
				var vm = new vm2_NodeVM({ sandbox : obj, console : "redirect", timeout : _gthis.timeout});
				...
				try {
					...
					if(response.length > 0 && data == 0) {
						interaction.reply({ allowedMentions : { parse : []}, embeds : [embed]}).then(function(succ) {
							haxe_Log.trace("" + interaction.user.tag + "(" + interaction.user.id + ") at " + format_date + " with file id: " + filename,{ fileName : "src/commands/Trace.hx", lineNumber : 254, className : "commands.Trace", methodName : "runCode"});
						},function(err) {
							haxe_Log.trace(err,{ fileName : "src/commands/Trace.hx", lineNumber : 258, className : "commands.Trace", methodName : "runCode"});
							$global.console.dir(err);
						});
						ls.kill();
						return;
					...
					interaction.reply({ allowedMentions : { parse : []}, content : mention + ("```\n" + compile_output + "```")}).then(null,function(err) {
						haxe_Log.trace(err,{ fileName : "src/commands/Trace.hx", lineNumber : 269, className : "commands.Trace", methodName : "runCode"});
					});
					haxe_Log.trace(e,{ fileName : "src/commands/Trace.hx", lineNumber : 270, className : "commands.Trace", methodName : "runCode"});
				}
			});
		});
	}
...
var commands_events_PinMessageInfo = function(_universe) {
	...
	this.table77d573418a21d66427b12280fbf7836b = this.universe.components.getTable(5);
	this.table87a8f92f715c03d0822a55d9b93a210d = this.universe.components.getTable(2);
};
	...
	,update: function(_dt) {
		...
		while(_g_active && _g_idx >= 0) {
			...
			if(command == "thread_pin_message") {
				...
				thread[0].send({ content : "<@" + thread[0].ownerId + "> You can pin messages in your own threads by Right clicking a message -> Apps -> Pin Message\n\n*This message will selfdestruct in 30 seconds.*"}).then((function(thread) {
					...
					return function(err) {
						haxe_Log.trace(err,{ fileName : "src/commands/events/PinMessageInfo.hx", lineNumber : 46, className : "commands.events.PinMessageInfo", methodName : "update"});
						$global.console.dir(err);
					};
				})());
				this.universe.deleteEntity(entity);
		...
		while(_g < _g1.length) {
			...
			message[0].delete().then((function(message) {
				...
				return function(err) {
					haxe_Log.trace(err,{ fileName : "src/commands/events/PinMessageInfo.hx", lineNumber : 61, className : "commands.events.PinMessageInfo", methodName : "update"});
				};
			})());
		}
	}
...
var components_CommandOptions = $hxEnums["components.CommandOptions"] = { __ename__:"components.CommandOptions",__constructs__:null
	...
	,SnippetEdit: ($_=function(id) { return {_hx_index:8,id:id,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.id];}}; },$_._hx_name="SnippetEdit",$_)
	,SnippetDelete: ($_=function(id) { return {_hx_index:9,id:id,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.id];}}; },$_._hx_name="SnippetDelete",$_)
	,SnippetSearch: ($_=function(taga,tagb,tagc) { return {_hx_index:10,taga:taga,tagb:tagb,tagc:tagc,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.taga,this.tagb,this.tagc];}}; },$_._hx_name="SnippetSearch",$_)
	,SnippetAdd: ($_=function(url,title,description,taga,tagb,tagc,tagd,tage) { return {_hx_index:11,url:url,title:title,description:description,taga:taga,tagb:tagb,tagc:tagc,tagd:tagd,tage:tage,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.url,this.title,this.description,this.taga,this.tagb,this.tagc,this.tagd,this.tage];}}; },$_._hx_name="SnippetAdd",$_)
	,Reminder: ($_=function(content,when,personal,thread_reply) { return {_hx_index:12,content:content,when:when,personal:personal,thread_reply:thread_reply,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.content,this.when,this.personal,this.thread_reply];}}; },$_._hx_name="Reminder",$_)
	,Social: ($_=function(tag,user) { return {_hx_index:13,tag:tag,user:user,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.tag,this.user];}}; },$_._hx_name="Social",$_)
	,Ban: ($_=function(user,reason,delete_messages) { return {_hx_index:14,user:user,reason:reason,delete_messages:delete_messages,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.user,this.reason,this.delete_messages];}}; },$_._hx_name="Ban",$_)
	,Say: ($_=function(message,message_id) { return {_hx_index:15,message:message,message_id:message_id,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.message,this.message_id];}}; },$_._hx_name="Say",$_)
	,React: ($_=function(message_id,emoji) { return {_hx_index:16,message_id:message_id,emoji:emoji,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.message_id,this.emoji];}}; },$_._hx_name="React",$_)
	,Helppls: ($_=function(topic) { return {_hx_index:17,topic:topic,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.topic];}}; },$_._hx_name="Helppls",$_)
	,Run: ($_=function(code) { return {_hx_index:18,code:code,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.code];}}; },$_._hx_name="Run",$_)
	,Trace: ($_=function(code) { return {_hx_index:19,code:code,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.code];}}; },$_._hx_name="Trace",$_)
	,Boop: ($_=function(user) { return {_hx_index:20,user:user,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.user];}}; },$_._hx_name="Boop",$_)
	,Poll: ($_=function(question,length,a,b,c,d,e,f,g,votes) { return {_hx_index:21,question:question,length:length,a:a,b:b,c:c,d:d,e:e,f:f,g:g,votes:votes,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.question,this.length,this.a,this.b,this.c,this.d,this.e,this.f,this.g,this.votes];}}; },$_._hx_name="Poll",$_)
	,Roundup: ($_=function(number) { return {_hx_index:22,number:number,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.number];}}; },$_._hx_name="Roundup",$_)
	,Rtfm: ($_=function(channel) { return {_hx_index:23,channel:channel,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.channel];}}; },$_._hx_name="Rtfm",$_)
	,Translate: ($_=function(to,message,from) { return {_hx_index:24,to:to,message:message,from:from,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.to,this.message,this.from];}}; },$_._hx_name="Translate",$_)
	,Helpdescription: ($_=function(description) { return {_hx_index:25,description:description,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.description];}}; },$_._hx_name="Helpdescription",$_)
	,Api: ($_=function(content,field) { return {_hx_index:26,content:content,field:field,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.content,this.field];}}; },$_._hx_name="Api",$_)
	,Notify: ($_=function(channel) { return {_hx_index:27,channel:channel,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.channel];}}; },$_._hx_name="Notify",$_)
	,Help: ($_=function(category) { return {_hx_index:28,category:category,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.category];}}; },$_._hx_name="Help",$_)
	,Haxelib: ($_=function(command) { return {_hx_index:29,command:command,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.command];}}; },$_._hx_name="Haxelib",$_)
	,QuoteList: ($_=function(user) { return {_hx_index:30,user:user,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.user];}}; },$_._hx_name="QuoteList",$_)
	,QuoteGet: ($_=function(name) { return {_hx_index:31,name:name,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.name];}}; },$_._hx_name="QuoteGet",$_)
	,QuoteDelete: ($_=function(name) { return {_hx_index:32,name:name,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.name];}}; },$_._hx_name="QuoteDelete",$_)
	,QuoteEdit: ($_=function(name) { return {_hx_index:33,name:name,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.name];}}; },$_._hx_name="QuoteEdit",$_)
	,QuoteCreate: ($_=function(name) { return {_hx_index:34,name:name,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.name];}}; },$_._hx_name="QuoteCreate",$_)
	,Mention: ($_=function(user,role) { return {_hx_index:35,user:user,role:role,__enum__:"components.CommandOptions",toString:$estr,__params__:function(){ return [this.user,this.role];}}; },$_._hx_name="Mention",$_)
	,Showcase: {_hx_name:"Showcase",_hx_index:36,__enum__:"components.CommandOptions",toString:$estr}
	,PinMessage: {_hx_name:"PinMessage",_hx_index:37,__enum__:"components.CommandOptions",toString:$estr}
	,Code: {_hx_name:"Code",_hx_index:38,__enum__:"components.CommandOptions",toString:$estr}
	,CodeLineNumbers: {_hx_name:"CodeLineNumbers",_hx_index:39,__enum__:"components.CommandOptions",toString:$estr}
};
components_CommandOptions.__constructs__ = [components_CommandOptions.Hi,components_CommandOptions.Threadcount,components_CommandOptions.Archive,components_CommandOptions.SnippetTags,components_CommandOptions.SnippetList,components_CommandOptions.Color,components_CommandOptions.TrackerCreate,components_CommandOptions.TrackerDelete,components_CommandOptions.SnippetEdit,components_CommandOptions.SnippetDelete,components_CommandOptions.SnippetSearch,components_CommandOptions.SnippetAdd,components_CommandOptions.Reminder,components_CommandOptions.Social,components_CommandOptions.Ban,components_CommandOptions.Say,components_CommandOptions.React,components_CommandOptions.Helppls,components_CommandOptions.Run,components_CommandOptions.Trace,components_CommandOptions.Boop,components_CommandOptions.Poll,components_CommandOptions.Roundup,components_CommandOptions.Rtfm,components_CommandOptions.Translate,components_CommandOptions.Helpdescription,components_CommandOptions.Api,components_CommandOptions.Notify,components_CommandOptions.Help,components_CommandOptions.Haxelib,components_CommandOptions.QuoteList,components_CommandOptions.QuoteGet,components_CommandOptions.QuoteDelete,components_CommandOptions.QuoteEdit,components_CommandOptions.QuoteCreate,components_CommandOptions.Mention,components_CommandOptions.Showcase,components_CommandOptions.PinMessage,components_CommandOptions.Code,components_CommandOptions.CodeLineNumbers];
...
$hxClasses["http.HttpClient"] = http_HttpClient;
...
http_HttpClient.prototype = {
		...
		return thenshim_Promise._new(function(resolve,reject) {
			...
			thenshim_Promise.then(_gthis.get_provider().makeRequest(request),function(response) {
				...
				if(_gthis.followRedirects && (response.httpStatus == 301 || response.httpStatus == 302)) {
					...
					http_Url.fromString(redirectLocation);
					if(redirectLocation == null) {
						var _this = _gthis.log;
				...
				_gthis._idToItem.remove(itemId);
				item.resolve(new http_HttpResult(_gthis,response));
				resolve(true);
			},function(error) {
	...
	,__properties__: {set_provider:"set_provider",get_provider:"get_provider",set_requestQueue:"set_requestQueue",get_requestQueue:"get_requestQueue"}
};
...
$hxClasses["http.HttpError"] = http_HttpError;
...
http_HttpError.prototype = {
	...
	,__properties__: {get_bodyAsJson:"get_bodyAsJson",get_bodyAsString:"get_bodyAsString"}
};
...
$hxClasses["http.HttpRequestObject"] = http_HttpRequestObject;
...
http_HttpRequestObject.prototype = {
	...
	,__properties__: {set_queryParams:"set_queryParams",get_queryParams:"get_queryParams"}
};
var http_HttpResponse = function() {
	this._body = null;
	this.buffer = null;
};
$hxClasses["http.HttpResponse"] = http_HttpResponse;
...
http_HttpResponse.prototype = {
	...
	,__properties__: {get_bodyAsJson:"get_bodyAsJson",get_bodyAsString:"get_bodyAsString",set_body:"set_body",get_body:"get_body"}
};
var http_HttpResult = function(client,response) {
	this.client = client;
	this.response = response;
};
$hxClasses["http.HttpResult"] = http_HttpResult;
http_HttpResult.__name__ = "http.HttpResult";
http_HttpResult.prototype = {
	client: null
	,response: null
	,__class__: http_HttpResult
};
var http_IHttpProvider = function() { };
$hxClasses["http.IHttpProvider"] = http_IHttpProvider;
...
$hxClasses["http.IHttpRequestTransformer"] = http_IHttpRequestTransformer;
...
$hxClasses["http.IHttpResponseTransformer"] = http_IHttpResponseTransformer;
...
$hxClasses["http.UrlObject"] = http_UrlObject;
...
$hxClasses["http.providers.DefaultHttpProvider"] = http_providers_DefaultHttpProvider;
...
$hxClasses["queues.QueueFactory"] = queues_QueueFactory;
queues_QueueFactory.__name__ = "queues.QueueFactory";
queues_QueueFactory.__properties__ = {get_instance:"get_instance"};
queues_QueueFactory.instance = null;
...
systems_MessageRouter.prototype = $extend(ecs_System.prototype,{
	update: function(_) {
		...
		while(_active && _g_idx >= 0) {
			...
			if(channel.id == "1234544675264925788") {
				...
				util_EcsTools.get_universe().components.set(entity1,2,"suggestion_box");
				util_EcsTools.get_universe().components.set(entity1,4,message);
				...
				var ecsTmpFamily2 = util_EcsTools.get_universe().families.get(4);
				if(bits_Bits.areSet(ecsEntCompFlags,ecsTmpFamily2.componentsMask)) {
					ecsTmpFamily2.add(entity1);
				}
				var ecsTmpFamily3 = util_EcsTools.get_universe().families.get(5);
				if(bits_Bits.areSet(ecsEntCompFlags,ecsTmpFamily3.componentsMask)) {
					ecsTmpFamily3.add(entity1);
				}
				var ecsTmpFamily4 = util_EcsTools.get_universe().families.get(2);
				if(bits_Bits.areSet(ecsEntCompFlags,ecsTmpFamily4.componentsMask)) {
					ecsTmpFamily4.add(entity1);
				}
				var ecsTmpFamily5 = util_EcsTools.get_universe().families.get(6);
				if(bits_Bits.areSet(ecsEntCompFlags,ecsTmpFamily5.componentsMask)) {
					ecsTmpFamily5.add(entity1);
				}
			}
			var entity2 = util_EcsTools.get_universe().createEntity();
			util_EcsTools.get_universe().components.set(entity2,2,"scam_prevention");
			util_EcsTools.get_universe().components.set(entity2,4,message);
			var ecsEntCompFlags1 = util_EcsTools.get_universe().components.flags[ecs_Entity.id(entity2)];
			var ecsTmpFamily6 = util_EcsTools.get_universe().families.get(1);
			if(bits_Bits.areSet(ecsEntCompFlags1,ecsTmpFamily6.componentsMask)) {
				ecsTmpFamily6.add(entity2);
			}
			var ecsTmpFamily7 = util_EcsTools.get_universe().families.get(3);
			if(bits_Bits.areSet(ecsEntCompFlags1,ecsTmpFamily7.componentsMask)) {
				ecsTmpFamily7.add(entity2);
			}
			var ecsTmpFamily8 = util_EcsTools.get_universe().families.get(4);
			if(bits_Bits.areSet(ecsEntCompFlags1,ecsTmpFamily8.componentsMask)) {
				ecsTmpFamily8.add(entity2);
			}
			var ecsTmpFamily9 = util_EcsTools.get_universe().families.get(5);
			if(bits_Bits.areSet(ecsEntCompFlags1,ecsTmpFamily9.componentsMask)) {
				ecsTmpFamily9.add(entity2);
			}
			var ecsTmpFamily10 = util_EcsTools.get_universe().families.get(2);
			if(bits_Bits.areSet(ecsEntCompFlags1,ecsTmpFamily10.componentsMask)) {
				ecsTmpFamily10.add(entity2);
			}
			var ecsTmpFamily11 = util_EcsTools.get_universe().families.get(6);
			if(bits_Bits.areSet(ecsEntCompFlags1,ecsTmpFamily11.componentsMask)) {
				ecsTmpFamily11.add(entity2);
			}
			var entity3 = util_EcsTools.get_universe().createEntity();
			util_EcsTools.get_universe().components.set(entity3,2,"keyword_tracker");
			util_EcsTools.get_universe().components.set(entity3,4,message);
			...
			var ecsTmpFamily13 = util_EcsTools.get_universe().families.get(3);
			if(bits_Bits.areSet(ecsEntCompFlags2,ecsTmpFamily13.componentsMask)) {
				ecsTmpFamily13.add(entity3);
			}
			var ecsTmpFamily14 = util_EcsTools.get_universe().families.get(4);
			if(bits_Bits.areSet(ecsEntCompFlags2,ecsTmpFamily14.componentsMask)) {
				ecsTmpFamily14.add(entity3);
			}
			var ecsTmpFamily15 = util_EcsTools.get_universe().families.get(5);
			if(bits_Bits.areSet(ecsEntCompFlags2,ecsTmpFamily15.componentsMask)) {
				ecsTmpFamily15.add(entity3);
			}
			var ecsTmpFamily16 = util_EcsTools.get_universe().families.get(2);
			if(bits_Bits.areSet(ecsEntCompFlags2,ecsTmpFamily16.componentsMask)) {
				ecsTmpFamily16.add(entity3);
			}
			var ecsTmpFamily17 = util_EcsTools.get_universe().families.get(6);
			if(bits_Bits.areSet(ecsEntCompFlags2,ecsTmpFamily17.componentsMask)) {
				ecsTmpFamily17.add(entity3);
			}
			this.universe.deleteEntity(entity);
		}
	}
	,publicThreadChannel: function(message) {
		if(StringTools.startsWith(message.content,"[showcase]")) {
			...
			util_EcsTools.get_universe().components.set(entity,4,message);
...
queues_QueueFactory._instance = null;
