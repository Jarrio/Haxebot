...
<<<<<<< HEAD
	...
	Main.universe.systems.add(new systems_commands_Session(Main.universe));
	...
	Main.client.once("ready",function() {
		...
		haxe_Log.trace("Ready!",{ fileName : "src/Main.hx", lineNumber : 53, className : "Main", methodName : "start"});
=======
	...
	s.onEnabled();
	var s = new systems_commands_mod_Ban(u);
	phase.systems[2] = s;
	...
	Main.client.on("messageCreate",function(message) {
		...
		if(channel.type == "DM" && !message.author.bot) {
			if(Object.prototype.hasOwnProperty.call(Main.dm_help_tracking.h,message.author.id)) {
				...
				Main.universe.components.set(_ecsTmpEntity,4,"helppls");
		...
		if(channel.type == "GUILD_TEXT" && !message.author.bot) {
			if(StringTools.startsWith(message.content,"!run")) {
				...
				var _ecsTmpEntity = Main.universe.createEntity();
				Main.universe.components.set(_ecsTmpEntity,0,code);
				Main.universe.components.set(_ecsTmpEntity,1,message);
				var ecsEntCompFlags = Main.universe.components.flags[ecs_Entity.id(_ecsTmpEntity)];
				var ecsTmpFamily = Main.universe.families.get(0);
				if(bits_Bits.areSet(ecsEntCompFlags,ecsTmpFamily.componentsMask)) {
			...
			if(StringTools.startsWith(message.content,"!react")) {
				var _ecsTmpEntity = Main.universe.createEntity();
				Main.universe.components.set(_ecsTmpEntity,4,"react");
				Main.universe.components.set(_ecsTmpEntity,1,message);
				var ecsEntCompFlags = Main.universe.components.flags[ecs_Entity.id(_ecsTmpEntity)];
				var ecsTmpFamily = Main.universe.families.get(2);
				if(bits_Bits.areSet(ecsEntCompFlags,ecsTmpFamily.componentsMask)) {
			...
			var _ecsTmpEntity = Main.universe.createEntity();
			Main.universe.components.set(_ecsTmpEntity,4,"scam_prevention");
			Main.universe.components.set(_ecsTmpEntity,1,message);
			var ecsEntCompFlags = Main.universe.components.flags[ecs_Entity.id(_ecsTmpEntity)];
			var ecsTmpFamily = Main.universe.families.get(2);
			if(bits_Bits.areSet(ecsEntCompFlags,ecsTmpFamily.componentsMask)) {
...
<<<<<<< HEAD
			haxe_Log.trace(interaction,{ fileName : "src/Main.hx", lineNumber : 111, className : "Main", methodName : "start"});
			haxe_Log.trace(enum_id,{ fileName : "src/Main.hx", lineNumber : 112, className : "Main", methodName : "start"});
...
<<<<<<< HEAD
		haxe_Log.trace(_g1.get_message(),{ fileName : "src/Main.hx", lineNumber : 129, className : "Main", methodName : "main"});
=======
...
<<<<<<< HEAD
	...
	rest.put(Routes.applicationGuildCommands(Main.config.client_id,Main.config.server_id),{ body : commands}).then(function(_) {
		haxe_Log.trace("Successfully registered application commands.",{ fileName : "src/Main.hx", lineNumber : 141, className : "Main", methodName : "main"});
	},function(err) {
		haxe_Log.trace(err,{ fileName : "src/Main.hx", lineNumber : 141, className : "Main", methodName : "main"});
	});
=======
...
Main.parseCommands = function() {
	...
	while(_g < command_defs.length) {
		...
		var main_command = new discord_$builder_SlashCommandBuilder().setName(command.name).setDescription(command.description).setDefaultPermission(permission);
...
<<<<<<< HEAD
	...
	,Session: ($_=function(type,id,name,description) { return {_hx_index:2,type:type,id:id,name:name,description:description,__enum__:"components.CommandOptions",toString:$estr}; },$_._hx_name="Session",$_.__params__ = ["type","id","name","description"],$_)
...
components_CommandOptions.__constructs__ = [components_CommandOptions.Hi,components_CommandOptions.Roundup,components_CommandOptions.Session,components_CommandOptions.Rtfm,components_CommandOptions.Api,components_CommandOptions.Notify,components_CommandOptions.Code,components_CommandOptions.Help,components_CommandOptions.Haxelib];
=======
	,Ban: ($_=function(user,reason,delete_messages) { return {_hx_index:1,user:user,reason:reason,delete_messages:delete_messages,__enum__:"components.CommandOptions",toString:$estr}; },$_._hx_name="Ban",$_.__params__ = ["user","reason","delete_messages"],$_)
	,React: ($_=function(emoji,message_id) { return {_hx_index:2,emoji:emoji,message_id:message_id,__enum__:"components.CommandOptions",toString:$estr}; },$_._hx_name="React",$_.__params__ = ["emoji","message_id"],$_)
	,Helppls: ($_=function(topic) { return {_hx_index:3,topic:topic,__enum__:"components.CommandOptions",toString:$estr}; },$_._hx_name="Helppls",$_.__params__ = ["topic"],$_)
	,Trace: ($_=function(code) { return {_hx_index:4,code:code,__enum__:"components.CommandOptions",toString:$estr}; },$_._hx_name="Trace",$_.__params__ = ["code"],$_)
	,Boop: ($_=function(user) { return {_hx_index:5,user:user,__enum__:"components.CommandOptions",toString:$estr}; },$_._hx_name="Boop",$_.__params__ = ["user"],$_)
...
systems_CommandDbBase.prototype = $extend(ecs_System.prototype,{
	...
	,get_name: null
	,err: function(err) {
		haxe_Log.trace(err,{ fileName : "src/systems/CommandDbBase.hx", lineNumber : 43, className : "systems.CommandDbBase", methodName : "err"});
	}
	,commands: null
	,table5d38588a6ddd880f90fc8234bccb893f: null
	,tablefa61f37a15ee60bbc1601eb42174bd3d: null
	...
	,run: function(command,interaction) {
		...
		if(Object.prototype.hasOwnProperty.call(Main.dm_help_tracking.h,interaction.user.id)) {
			...
			while(_g < _g1.length) {
				...
				interaction.reply({ content : content.content.toString()}).then(null,$bind(this,this.err));
				break;
			}
			return;
		}
...
>>>>>>> master
			...
			interaction.reply(msg).then(null,$bind(this,this.err));
		}
	}
	,get_name: function() {
		return "help";
...
systems_commands_Helpdescription.prototype = $extend(systems_CommandDbBase.prototype,{
	...
	,findThread: function(interaction,description) {
		...
		if(topic == null) {
			interaction.reply("This channel is not a valid topic. Did you run the command from a valid thread?").then(null,$bind(this,this.err));
			return;
		}
...
var systems_commands_Run = function(_universe) {
	...
	ecs_System.call(this,_universe);
	this.code_messages = this.universe.families.get(0);
	this.table25aaa7d3f4989532034d5f61746d0948 = this.universe.components.getTable(0);
	this.tabled1cd3067ebd0108e92f1425a40ea7b45 = this.universe.components.getTable(1);
};
...
systems_commands_Run.prototype = $extend(ecs_System.prototype,{
	...
	,extractCode: function(message,response) {
		...
		check_code = new EReg("!run[\\s|\n| \n](.*)","gmis");
		if(check_code.match(message)) {
			haxe_Log.trace(check_code.matched(1),{ fileName : "src/systems/commands/Run.hx", lineNumber : 97, className : "systems.commands.Run", methodName : "extractCode"});
	...
	,deleteFile: function(filename) {
		try {
			...
			haxe_NativeStackTrace.lastError = _g;
			var _g1 = haxe_Exception.caught(_g).unwrap();
			haxe_Log.trace(_g1,{ fileName : "src/systems/commands/Run.hx", lineNumber : 109, className : "systems.commands.Run", methodName : "deleteFile"});
		}
	}
	...
	,runCodeOnThread: function(code,message) {
		...
		try {
			...
			js_node_Fs.appendFile("" + this.get_base_path() + "/hx/" + filename + ".hx",code_content + ("//User:" + message.author.tag + " | time: " + Std.string(new Date())),function(error) {
				if(error != null) {
					haxe_Log.trace(error,{ fileName : "src/systems/commands/Run.hx", lineNumber : 284, className : "systems.commands.Run", methodName : "runCodeOnThread"});
				}
				...
				ls.stderr.once("data",function(data) {
					haxe_Log.trace("error: " + data,{ fileName : "src/systems/commands/Run.hx", lineNumber : 309, className : "systems.commands.Run", methodName : "runCodeOnThread"});
				...
				ls.once("close",function(data) {
					...
					if(!sys_FileSystem.exists(js_file)) {
						haxe_Log.trace("Code likely errored and didnt compile (" + filename + ".js)",{ fileName : "src/systems/commands/Run.hx", lineNumber : 320, className : "systems.commands.Run", methodName : "runCodeOnThread"});
						ls.kill("SIGTERM");
						return;
					}
					var obj = null;
					...
					try {
						...
						if(response.length > 0 && data == 0) {
							message.reply({ embeds : [embed]}).then(function(succ) {
								haxe_Log.trace("" + message.author.tag + " at " + format_date + " with file id: " + filename,{ fileName : "src/systems/commands/Run.hx", lineNumber : 397, className : "systems.commands.Run", methodName : "runCodeOnThread"});
						...
						message.reply({ content : mention + ("```\n" + compile_output + "```")});
						haxe_Log.trace(_g1,{ fileName : "src/systems/commands/Run.hx", lineNumber : 406, className : "systems.commands.Run", methodName : "runCodeOnThread"});
					}
				});
			});
			return;
			...
			haxe_NativeStackTrace.lastError = _g;
			var _g1 = haxe_Exception.caught(_g).unwrap();
			haxe_Log.trace(_g1,{ fileName : "src/systems/commands/Run.hx", lineNumber : 413, className : "systems.commands.Run", methodName : "runCodeOnThread"});
...
<<<<<<< HEAD
var systems_commands_Session = function(_universe) {
	this.rooms = new haxe_ds_StringMap();
	systems_CommandBase.call(this,_universe);
};
$hxClasses["systems.commands.Session"] = systems_commands_Session;
systems_commands_Session.__name__ = "systems.commands.Session";
systems_commands_Session.__super__ = systems_CommandBase;
systems_commands_Session.prototype = $extend(systems_CommandBase.prototype,{
	rooms: null
	,onAdded: function() {
	...
	,run: function(command,interaction) {
		...
		if(_g._hx_index == 2) {
			var _g1 = _g.id;
			var _g2 = _g.name;
			var _g3 = _g.description;
			switch(_g.type) {
			case "create":
				this.create(_g1,interaction,_g2,_g3);
				break;
			case "join":
				this.join(_g1,interaction);
				break;
			case "leave":
				this.leave(_g1,interaction);
				break;
			default:
			}
		}
	}
	,create: function(id,message,name,description) {
		if(Object.prototype.hasOwnProperty.call(this.rooms.h,id)) {
			message.reply("A room with that ID already exists.");
			return;
		}
		this.rooms.h[id] = new systems_types_Room(id,message.user,name,description);
		message.reply("User " + message.user.tag + " created room(#" + id + ") " + name);
	}
	,join: function(id,message) {
		var room = this.rooms.h[id];
		if(room != null) {
			...
			room.addMember(message.user);
			haxe_Log.trace("member " + message.user.tag + " joined the room " + id,{ fileName : "src/systems/commands/Session.hx", lineNumber : 50, className : "systems.commands.Session", methodName : "join"});
			haxe_Log.trace(this.rooms.h[id].members.length,{ fileName : "src/systems/commands/Session.hx", lineNumber : 51, className : "systems.commands.Session", methodName : "join"});
		}
	}
	,leave: function(id,message) {
		var room = this.rooms.h[id];
		if(room.memberExists(message.user)) {
	...
	,__class__: systems_commands_Session
});
var systems_types_Room = function(id,user,name,description,hscript) {
	if(hscript == null) {
		hscript = "";
	}
	this.id = id;
...
$hxClasses["systems.types.Room"] = systems_types_Room;
systems_types_Room.__name__ = "systems.types.Room";
systems_types_Room.prototype = {
	id: null
	,name: null
	,description: null
	,members: null
	,hscript: null
	,modScript: function(hscript) {
		this.hscript = hscript;
	}
	,addMember: function(user) {
		this.members.push(user);
	}
	,memberExists: function(user) {
		var _g = 0;
	...
	,removeMember: function(user) {
		haxe_Log.trace(this.members.length,{ fileName : "src/systems/types/Room.hx", lineNumber : 37, className : "systems.types.Room", methodName : "removeMember"});
		HxOverrides.remove(this.members,user);
		haxe_Log.trace(this.members.length,{ fileName : "src/systems/types/Room.hx", lineNumber : 39, className : "systems.types.Room", methodName : "removeMember"});
		return false;
	}
	,get_owner: function() {
		return this.members[0];
	}
	,__class__: systems_types_Room
...
systems_commands_ScamPrevention.prototype = $extend(systems_CommandBase.prototype,{
		...
		this.timeoutUser(message,function(_) {
			message.reply({ content : "<@&198916468312637440> Please review this message by: <@" + message.author.id + ">", embeds : [embed]}).then(function(_) {
				...
				while(_g < messages.length) {
					...
					++_g;
					message.delete().then(null,$bind(_gthis,_gthis.err));
				}
			},$bind(_gthis,_gthis.err));
		});
	}
	...
	,timeoutUser: function(message,callback) {
		...
		message.guild.members.fetch(message.author.id).then(function(guild_member) {
			...
			guild_member.timeout(43200000,"You are spamming something that doesn\t need to be spammed. Wait for review.").then(callback,$bind(_gthis,_gthis.err));
	...
	,banUser: function(message,callback) {
		...
		message.guild.members.fetch(message.author.id).then(function(guild_member) {
			...
			message.channel.send("User <@" + message.author.id + "> has been auto banned for sending scam links.").then(callback,$bind(_gthis,_gthis.err));
		},$bind(this,this.err));
	}
...
systems_commands_Trace.prototype = $extend(systems_CommandBase.prototype,{
	...
	,update: function(_) {
		...
		if(!(now - this.last_cleared < 604800000)) {
			...
			try {
				...
				haxe_Log.trace(path,{ fileName : "src/systems/commands/Trace.hx", lineNumber : 47, className : "systems.commands.Trace", methodName : "cleanDirectory"});
				var _g = 0;
				...
				var _g1 = haxe_Exception.caught(_g);
				haxe_Log.trace(_g1,{ fileName : "src/systems/commands/Trace.hx", lineNumber : 57, className : "systems.commands.Trace", methodName : "cleanDirectory"});
			}
		}
	}
	...
	,runCode: function(code,interaction) {
		...
		js_node_Fs.appendFile("" + this.get_base_path() + "/hx/" + filename + ".hx",final_code + ("\n//User:" + interaction.user.tag + " id: " + interaction.user.id + "| time: " + Std.string(new Date())),function(error) {
			if(error != null) {
				haxe_Log.trace(error,{ fileName : "src/systems/commands/Trace.hx", lineNumber : 80, className : "systems.commands.Trace", methodName : "runCode"});
			}
			var commands = ["-cp","" + _gthis.get_base_path() + "/hx","-main",filename,"-js","" + _gthis.get_base_path() + "/bin/" + filename + ".js"];
			var $process = "./haxe/haxe";
			if(!sys_FileSystem.exists("./haxe/haxe")) {
			...
			ls.stderr.once("data",function(data) {
				haxe_Log.trace("error: " + data,{ fileName : "src/systems/commands/Trace.hx", lineNumber : 100, className : "systems.commands.Trace", methodName : "runCode"});
			...
			ls.once("close",function(data) {
				...
				var js_file = "" + _gthis.get_base_path() + "/bin/" + filename + ".js";
				if(!sys_FileSystem.exists(js_file)) {
					haxe_Log.trace("Code likely errored and didnt compile (" + filename + ".js)",{ fileName : "src/systems/commands/Trace.hx", lineNumber : 109, className : "systems.commands.Trace", methodName : "runCode"});
					return;
				}
				var obj = null;
				var vm = new vm2_NodeVM({ sandbox : obj, console : "redirect", timeout : _gthis.timeout});
				...
				try {
					...
					if(response.length > 0 && data == 0) {
						interaction.reply({ embeds : [embed]}).then(function(succ) {
							haxe_Log.trace("" + interaction.user.tag + "(" + interaction.user.id + ") at " + format_date + " with file id: " + filename,{ fileName : "src/systems/commands/Trace.hx", lineNumber : 176, className : "systems.commands.Trace", methodName : "runCode"});
						},null);
						ls.kill();
						return;
					}
					...
					haxe_Log.trace(_g1,{ fileName : "src/systems/commands/Trace.hx", lineNumber : 184, className : "systems.commands.Trace", methodName : "runCode"});
				}
			});
		});
	}
...
$hxClasses["systems.commands.mod.Ban"] = systems_commands_mod_Ban;
systems_commands_mod_Ban.__name__ = "systems.commands.mod.Ban";
systems_commands_mod_Ban.__super__ = systems_CommandBase;
systems_commands_mod_Ban.prototype = $extend(systems_CommandBase.prototype,{
	run: function(command,interaction) {
		var _gthis = this;
		var _g = command.content;
		if(_g._hx_index == 1) {
			...
			var delete_messages = _g.delete_messages;
			var member = interaction.channel.guild.members.cache.get(user.id);
			if(member == null) {
				interaction.reply("User not found.");
				return;
			}
			member.guild.channels.fetch(null,{ force : true}).then(function(response) {
				var channels = [];
				var history = [];
				var jsIterator = response.values();
				var _g_lastStep = jsIterator.next();
				while(!_g_lastStep.done) {
					var v = _g_lastStep.value;
					_g_lastStep = jsIterator.next();
					if(v.type != "GUILD_TEXT") {
						continue;
					}
					channels.push(v);
				}
				var _g1_current = 0;
				var _g1_array = channels;
				while(_g1_current < _g1_array.length) {
					var _g2_value = _g1_array[_g1_current];
					var _g2_key = _g1_current++;
					var key = [_g2_key];
					if(_g2_value.type != "GUILD_TEXT") {
						break;
					}
					Main.client.channels.fetch(_g2_value.id,{ force : true}).then((function(key) {
						return function(channel) {
							channel.messages.fetch(null,{ force : true}).then((function(key) {
								return function(response) {
									var count = 0;
									var messages = response;
									var jsIterator = messages.values();
									var _g3_lastStep = jsIterator.next();
									while(!_g3_lastStep.done) {
										var v = _g3_lastStep.value;
										_g3_lastStep = jsIterator.next();
									...
									if(key[0] + 1 == channels.length) {
										var log = [];
										var _g = 0;
										while(_g < history.length) {
											var message = history[_g];
											++_g;
											if(message.content == null || message.content.length == 0) {
												continue;
											}
											log.push({ user_tag : message.author.tag, user_id : message.author.id, message : message.content, timestamp : message.createdTimestamp, user_joined : message.member.joinedTimestamp, channel : message.channel.name});
										}
										...
										js_node_Fs.writeFileSync("./commands/ban_log.json",JSON.stringify(log));
										util_DiscordUtil.getChannel("952952631079362650",(function() {
											return function(channel) {
												var embed = new discord_$js_MessageEmbed();
												...
												interaction.reply({ embeds : [embed]}).then(null,$bind(_gthis,_gthis.err));
												var files = null;
												if(log.length > 0) {
													files = ["./commands/ban_log.json"];
												}
												channel.send({ embeds : [embed], files : files}).then((function() {
													return function(_) {
														var days = Std.parseInt(delete_messages);
														if(days == null) {
															days = 1;
														}
	...
	,update: function(_) {
		...
		if(tmp) {
			...
			var command = Main.getCommand(this.get_name());
			if(command != null) {
				util_DiscordUtil.setCommandPermission(command,[{ id : "198916468312637440", type : "ROLE", permission : true},{ id : "738508312382799874", type : "ROLE", permission : true}]);
			}
		}
	}
	,get_name: function() {
	...
	,__class__: systems_commands_mod_Ban
});
...
Main.commands = new haxe_ds_StringMap();
