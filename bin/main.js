...
Main.start = function() {
	...
	Main.client.once("ready",function() {
		...
		haxe_Log.trace("Ready!",{ fileName : "src/Main.hx", lineNumber : 102, className : "Main", methodName : "start"});
		...
		res.then(function(foo) {
			...
			while(_g < foo.length) {
				var item = foo[_g];
				++_g;
				haxe_Log.trace("DEBUG - " + Std.string(item.name) + " is REGISTERED",{ fileName : "src/Main.hx", lineNumber : 111, className : "Main", methodName : "start"});
			}
		},Util_err);
	});
	...
	Main.client.on("ChatInputAutoCompleteEvent",function(incoming) {
		haxe_Log.trace("disconnected",{ fileName : "src/Main.hx", lineNumber : 157, className : "Main", methodName : "start"});
		haxe_Log.trace(incoming,{ fileName : "src/Main.hx", lineNumber : 158, className : "Main", methodName : "start"});
	});
	Main.client.on("interactionCreate",function(interaction) {
		...
		if(interaction.isModalSubmit()) {
			...
			case "quote_set":
				...
				break;
			default:
				haxe_Log.trace(interaction.customId + " - unhandled model",{ fileName : "src/Main.hx", lineNumber : 180, className : "Main", methodName : "start"});
			}
			return;
		}
...
Main.saveCommand = function(command) {
	...
	haxe_Log.trace("registered " + command.name,{ fileName : "src/Main.hx", lineNumber : 277, className : "Main", methodName : "saveCommand"});
};
Main.main = function() {
	try {
		...
		haxe_Log.trace(_g1.get_message(),{ fileName : "src/Main.hx", lineNumber : 284, className : "Main", methodName : "main"});
	}
	var token = "";
	...
	firebase_web_auth_Auth.signInWithEmailAndPassword(firebase_web_auth_Auth.getAuth(),Main.config.username,Main.config.password).then(function(res) {
		haxe_Log.trace("logged in",{ fileName : "src/Main.hx", lineNumber : 299, className : "Main", methodName : "main"});
		Main.auth = res.user;
		Main.logged_in = true;
	},Util_err);
	Main.start();
...
function Util_loadFile(filename,pos) {
	...
	try {
		...
		haxe_Log.trace("Failed to load file or parse json",{ fileName : "src/Util.hx", lineNumber : 15, className : "_Util.Util_Fields_", methodName : "loadFile", customParams : [pos]});
	}
	return data;
}
...
function Util_err(err,pos) {
	haxe_Log.trace(err,{ fileName : "src/Util.hx", lineNumber : 47, className : "_Util.Util_Fields_", methodName : "err", customParams : [pos]});
}
var bits_Bits = {};
			...
			case "delete":
				firebase_web_firestore_Firestore.getDocs(query).then(function(res) {
					...
					if(res.docs.length > 1) {
						...
						haxe_Log.trace(name,{ fileName : "src/systems/commands/Quote.hx", lineNumber : 211, className : "systems.commands.Quote", methodName : "run"});
						haxe_Log.trace(interaction.user.id,{ fileName : "src/systems/commands/Quote.hx", lineNumber : 212, className : "systems.commands.Quote", methodName : "run"});
						return;
					}
			...
			js_node_Fs.appendFile("" + this.get_base_path() + "/hx/" + filename + ".hx",code_content + ("//User:" + message.author.tag + " | time: " + Std.string(new Date())),function(error) {
				...
				ls.once("close",function(data) {
					...
					try {
						...
						message.reply({ content : mention + ("```\n" + compile_output + "```")});
						haxe_Log.trace(_g1,{ fileName : "src/systems/commands/Run.hx", lineNumber : 450, className : "systems.commands.Run", methodName : "runCodeOnThread"});
					}
				});
			});
			return;
...
util_DiscordUtil.setCommandPermission = function(command,permissions,succ,fail) {
	command.permissions.set({ guild : Main.guild_id, command : command.id, permissions : permissions}).then(function(_) {
		...
		haxe_Log.trace(err,{ fileName : "src/Util.hx", lineNumber : 47, className : "_Util.Util_Fields_", methodName : "err", customParams : [{ fileName : "src/util/DiscordUtil.hx", lineNumber : 28, className : "util.DiscordUtil", methodName : "setCommandPermission"}]});
