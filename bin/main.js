...
commands_ScamPrevention.prototype = $extend(systems_CommandBase.prototype,{
	...
	,update: function(_) {
		...
		if(!(new Date().getTime() - this.phishing_update_time < 21600000)) {
			...
			links.onData = function(data) {
				try {
					...
					var _g1 = haxe_Exception.caught(_g);
					haxe_Log.trace(_g1,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 227, className : "commands.ScamPrevention", methodName : "getPhishingLinks"});
					haxe_Log.trace("error parsing phishing links",{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 228, className : "commands.ScamPrevention", methodName : "getPhishingLinks"});
		...
		while(_g_current < _g_length) {
			var key = _g_keys[_g_current++];
			var _g_value = h[key];
			haxe_Log.trace(_g_value.createdTimestamp,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 172, className : "commands.ScamPrevention", methodName : "update"});
		...
		this.timeoutUser(message,function(_) {
			message.reply({ content : "<@&198916468312637440> Please review this message by: <@" + message.author.id + ">", embeds : [embed]}).then(function(_) {
				...
				while(_g < messages.length) {
					...
					++_g;
					message.delete().then(null,function(err) {
						haxe_Log.trace(err,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 200, className : "commands.ScamPrevention", methodName : "reviewMessage"});
						$global.console.dir(err);
					});
				}
			},function(err) {
	...
	,timeoutUser: function(message,callback) {
		...
		message.guild.members.fetch(message.author.id).then(function(guild_member) {
			...
			guild_member.timeout(43200000,"Stop spamming, a mod will review this at their convenience.").then(callback,function(err) {
				haxe_Log.trace(err,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 239, className : "commands.ScamPrevention", methodName : "timeoutUser"});
				$global.console.dir(err);
			});
	...
	,banUser: function(messages,callback) {
		...
		message.guild.members.fetch(message.author.id).then(function(guild_member) {
			...
			guild_member.ban({ days : 1, reason : "found phishing links, auto banned."}).then(null,function(err) {
				haxe_Log.trace(err,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 273, className : "commands.ScamPrevention", methodName : "banUser"});
				$global.console.dir(err);
			});
			var id = message.author.id;
			var _this = _gthis.time_since;
			...
			haxe_Log.trace(err,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 282, className : "commands.ScamPrevention", methodName : "banUser"});
			$global.console.dir(err);
		});
	}
	,logMessage: function(id,embed,action) {
		...
		Main.client.channels.fetch("952952631079362650").then(function(channel) {
			...
			haxe_Log.trace(err,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 293, className : "commands.ScamPrevention", methodName : "logMessage"});
			$global.console.dir(err);
		});
	}
				...
				if(message.content.indexOf(link) != -1) {
					...
					if(regex.match(message.content)) {
						...
						if(url.hostname.length == 0 || url.hostname == null) {
							haxe_Log.trace(regex.matched(1),{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 338, className : "commands.ScamPrevention", methodName : "checkPhishingLinks"});
							return false;
						}
	...
	,checkEquality: function(messages) {
		...
		while(_g < messages.length) {
			...
			try {
				...
				haxe_Log.trace(_g2,{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 388, className : "commands.ScamPrevention", methodName : "checkEquality"});
				haxe_Log.trace(JSON.stringify(messages),{ fileName : "src/commands/ScamPrevention.hx", lineNumber : 389, className : "commands.ScamPrevention", methodName : "checkEquality"});
			}
		}
